...
namespace SeekASoul
{
	namespace Gameplay
	{
		...
		public:
			...
			const std::vector<Engine::BoxCollideable*> GetNearbyObjects(const sf::FloatRect& boundingBox) const;

			inline void AddObjectOnMap(Engine::BoxCollideable* object)
			{
		...
		private:
			...
			{
				bool operator()(const Engine::BoxCollideable* a, const Engine::BoxCollideable* b) const 
				{
			...
			std::set<Engine::BoxCollideable*, BoxComparatorX> m_ObjectsOnMapX;
			std::set<Engine::BoxCollideable*, BoxComparatorY> m_ObjectsOnMapY;
